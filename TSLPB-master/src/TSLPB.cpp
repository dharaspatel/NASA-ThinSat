...
void TSLPB::sleepWithWakeOnSerialReady() {
    ...
    sleep_enable();
//    attachInterrupt(digitalPinToInterrupt(TSL_NSL_BUS_STATUS_PIN), wakeOnSerialReady, NSL_SERIAL_READY);
    sleep_mode();
    sleep_disable();
    detachInterrupt(digitalPinToInterrupt(TSL_NSL_BUS_STATUS_PIN));
}
...
{
    if (digitalRead(TSL_NSL_BUS_STATUS_PIN) == NSL_SERIAL_READY) {
        return true;
    } else {
        return false;
    }
